...
class INESDataConnectorsAdapter:
    ...
    def wait_for_protocol_api_ready(self, connector_name, timeout=180, poll_interval=3):
        print(f"Waiting for protocol API to be ready: {connector_name}")
        start = time.time()
        base_url = self.connector_base_url(connector_name)
        url = f"{base_url}/protocol/catalog/request"
        payload = {
            "@context": {
                "@vocab": "https://w3id.org/edc/v0.0.1/ns/"
            }
        ...
        while time.time() - start <= timeout:
            try:
                ...
                if response.status_code < 500 and response.status_code != 404:
                    print(f"Protocol API ready: {connector_name} (HTTP {response.status_code})")
                    return True
                last_issue = f"HTTP {response.status_code}"
            except Exception as exc:
                last_issue = str(exc)
        ...
        if last_issue:
            print(f"Protocol API not ready for {connector_name}: {last_issue}")
        else:
            print(f"Protocol API not ready for {connector_name}")
        return False
    ...
    def validate_connectors_deployment(self, connectors):
        ...
        for connector in connectors:
            ...
            print(f"Checking Protocol API availability: {connector}")
            if not self.wait_for_protocol_api_ready(connector):
                print(f"Protocol API not reachable: {connector}")
                return False
    ...
    def deploy_connectors(self):
        ...
        connector_hosts = []
        for ds in dataspaces:
            connector_hosts.extend(self.config_adapter.generate_hosts(ds["name"]))
        connector_hosts.extend(self.config_adapter.generate_connector_hosts(all_connectors))
        connector_hosts = list(dict.fromkeys(connector_hosts))
