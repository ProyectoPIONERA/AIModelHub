...
class INESDataDeploymentAdapter:
    ...
    def _rollout_timeout_seconds(self):
        raw_value = os.environ.get("INESDATA_ROLLOUT_TIMEOUT_SECONDS", "").strip()
        if not raw_value:
            return 900
        try:
            return max(1, int(raw_value))
        except ValueError:
            print(f"Invalid INESDATA_ROLLOUT_TIMEOUT_SECONDS value '{raw_value}'; using 900s")
            return 900
    ...
    def restart_registration_service(self):
        ...
        timeout_seconds = self._rollout_timeout_seconds()
        ...
        rollout_output = self.run(
            f"kubectl rollout status deployment/{deployment_name} -n {namespace} --timeout={timeout_seconds}s",
            capture=True,
            check=False,
        )
    ...
    def restart_public_portal_backend(self):
        deployment_name = f"{self._dataspace_name()}-public-portal-backend"
        namespace = self.config.namespace_demo()
        timeout_seconds = self._rollout_timeout_seconds()

        print("\nRestarting public-portal-backend deployment to pick up recreated dataspace credentials...")
        if self.run(
            f"kubectl rollout restart deployment/{deployment_name} -n {namespace}",
            check=False,
        ) is None:
            self._fail("Could not restart public-portal-backend deployment")
        ...
        if rollout_output is None:
            self._fail("public-portal-backend deployment did not finish rolling out")
        self._print_unique_lines(rollout_output)

    @staticmethod
    def _decode_secret_data(secret_payload):
        decoded = {}
        for key, value in (secret_payload.get("data") or {}).items():
            try:
    ...
    def _expected_public_portal_secret_data(self, values_file):
        with open(values_file, encoding="utf-8") as handle:
            values = yaml.safe_load(handle) or {}

        backend = values.get("backend") or {}
        services = values.get("services") or {}
        portal_db = (services.get("db") or {}).get("portal") or {}
        jwt_secret = (backend.get("jwt") or {}).get("secret") or {}
        api_token = (backend.get("api") or {}).get("token") or {}
        oauth2 = backend.get("oauth2") or {}

        return {
            "DATABASE_USERNAME": str(portal_db.get("user") or ""),
            "DATABASE_PASSWORD": str(portal_db.get("password") or ""),
            "JWT_SECRET": str(jwt_secret.get("core") or ""),
            "ADMIN_JWT_SECRET": str(jwt_secret.get("admin") or ""),
            "APP_KEYS": str((backend.get("app") or {}).get("keys") or ""),
            "API_TOKEN_SALT": str(api_token.get("salt") or ""),
            "TOKEN_USERNAME": str(oauth2.get("username") or ""),
            "TOKEN_PASSWORD": str(oauth2.get("password") or ""),
        }

    def _public_portal_release_needs_reconcile(self, values_file):
        namespace = self.config.namespace_demo()
        backend_deployment = f"{self._dataspace_name()}-public-portal-backend"
        secret_name = f"{self._dataspace_name()}-public-portal-backend-secret"

        deployment_result = self.run(
            f"kubectl get deployment/{backend_deployment} -n {namespace} -o name",
            capture=True,
            check=False,
        )
        if deployment_result is None:
            print("Public portal backend deployment is missing; reconciliation required.")
            return True

        secret_output = self.run(
            f"kubectl get secret {secret_name} -n {namespace} -o json",
            capture=True,
            check=False,
        )
        if secret_output is None:
            print("Public portal backend secret is missing; reconciliation required.")
            return True

        try:
            current_secret = self._decode_secret_data(json.loads(secret_output))
        except json.JSONDecodeError:
            print("Public portal backend secret could not be parsed; reconciliation required.")
            return True

        expected_secret = self._expected_public_portal_secret_data(values_file)
        for key, expected_value in expected_secret.items():
            if current_secret.get(key) != expected_value:
                print(f"Public portal backend secret is out of sync for key '{key}'; reconciliation required.")
                return True

        return False

    def _reconcile_public_portal_release_if_needed(self):
        values_file = self.config.public_portal_values_file()
        if not os.path.exists(values_file):
            self._fail("Public portal values file not found")

        if not self._public_portal_release_needs_reconcile(values_file):
            print("Public portal release already matches the current dataspace credentials.")
            return

        print("\nDeploying public-portal release to refresh dataspace credentials...")
        if not self.infrastructure.deploy_helm_release(
            self.config.helm_release_pp(),
            self.config.namespace_demo(),
            os.path.basename(values_file),
            cwd=self.config.public_portal_dir(),
        ):
            self._fail("Error deploying public-portal release")

        self.restart_public_portal_backend()

    def _wait_for_dataspace_deployments(self):
        namespace = self.config.namespace_demo()
        timeout_seconds = self._rollout_timeout_seconds()
        deployment_names = (
            f"{self._dataspace_name()}-registration-service",
            f"{self._dataspace_name()}-public-portal-backend",
            f"{self._dataspace_name()}-public-portal-frontend",
        )

        print("\nWaiting for dataspace deployments to finish rolling out...")
        for deployment_name in deployment_names:
            ...
            if rollout_output is None:
                print(f"Dataspace deployment did not become ready: {deployment_name}")
                self.run(f"kubectl get deployment/{deployment_name} -n {namespace}", check=False)
                return False
            self._print_unique_lines(rollout_output)
    ...
    def deploy_dataspace(self):
        ...
        self._reconcile_public_portal_release_if_needed()

        if not self._wait_for_dataspace_deployments():
