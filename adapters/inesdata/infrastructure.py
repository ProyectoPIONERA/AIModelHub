...
class INESDataInfrastructureAdapter:
    ...
    @staticmethod
    def _vault_status_command(pod_name, namespace):
        return f"kubectl exec {pod_name} -n {namespace} -- vault status -format=json"

    def _read_vault_status(self, pod_name, namespace):
        command = self._vault_status_command(pod_name, namespace)
        ...
        if result.returncode not in (0, 2):
            ...
            return None, f"vault status returned exit code {result.returncode}"

        if not stdout_text:
            return None, diagnostic or "vault status produced no JSON output"

        try:
            return json.loads(stdout_text), None
        except json.JSONDecodeError as exc:
            if diagnostic:
                return None, f"{diagnostic}\nJSON decode error: {exc}"
            return None, f"JSON decode error: {exc}"

    def _load_vault_keys(self):
        vault_file_path = self.config.vault_keys_path()

        try:
            with open(vault_file_path, "r", encoding="utf-8") as handle:
                keys = json.load(handle)
        except FileNotFoundError:
            return None, "Vault keys file not found"
        except json.JSONDecodeError:
            return None, "Vault keys file corrupted"
        except OSError as exc:
            return None, f"Could not read Vault keys file: {exc}"

        return keys, None

    @staticmethod
    def _extract_vault_keys(keys):
        unseal_key = (keys or {}).get("unseal_keys_hex", [None])[0]
        root_token = (keys or {}).get("root_token")
        return unseal_key, root_token

    def _wait_for_vault_ready(self, pod_name, namespace, timeout, poll_interval):
        deadline = time.time() + max(int(timeout), 1)

        while time.time() <= deadline:
            status_data, _error = self._read_vault_status(pod_name, namespace)
            if status_data and status_data.get("initialized") and not status_data.get("sealed"):
    ...
    def setup_vault(self, namespace=None):
        ...
        if status_data:
            ...
            print(
                "Vault status unavailable. Reusing existing Vault keys and assuming the pod needs unseal. "
                f"Details: {status_error or 'no additional diagnostics'}"
            )
            initialized = True
            sealed = True
        else:
            print(f"Could not get Vault status: {status_error or 'no additional diagnostics'}")
            return False
        ...
        keys, keys_error = self._load_vault_keys()
        if not keys:
            print(f"Error: {keys_error}")
            return False

        unseal_key, root_token = self._extract_vault_keys(keys)
        ...
        final_status_data, final_status_error = self._read_vault_status(pod_name, namespace)
        if not final_status_data:
            print(f"Error: Could not get final Vault status: {final_status_error or 'no additional diagnostics'}")
            return False

        try:
            initialized = final_status_data.get("initialized", False)
            sealed = final_status_data.get("sealed", True)
            print("\nVault final status:")
    ...
    def ensure_vault_unsealed(self, timeout=30, poll_interval=2):
        ...
        status_data, status_error = self._read_vault_status(pod, self.config.NS_COMMON)
        keys, keys_error = self._load_vault_keys()

        if status_data and not status_data.get("initialized"):
        ...
        if status_data is None:
            if not keys:
                ...
                return False
            print(
                "Vault status unavailable. Attempting unseal using existing keys. "
                f"Details: {status_error or 'no additional diagnostics'}"
            )
            should_unseal = True
        elif status_data.get("sealed"):
            should_unseal = True

        if should_unseal:
            print("Vault sealed. Running unseal...")
            if not keys:
                print(f"Vault keys unavailable: {keys_error}")
                return False
            unseal_key, _root_token = self._extract_vault_keys(keys)
            if not unseal_key:
                print("Vault keys file does not contain an unseal key")
                return False
        ...
        return self._wait_for_vault_ready(pod, self.config.NS_COMMON, timeout, poll_interval)
    ...
    @staticmethod
    def _filter_relevant_pods_by_prefixes(snapshot, pod_name_prefixes=None):
        prefixes = tuple(prefix for prefix in (pod_name_prefixes or ()) if prefix)
        if not prefixes:
            return snapshot
        return [pod for pod in snapshot if any(pod["name"].startswith(prefix) for prefix in prefixes)]

    def wait_for_namespace_stability(self, namespace, duration=15, poll_interval=3, pod_name_prefixes=None):
        ...
        while time.time() < deadline:
            ...
            relevant_pods = self._filter_relevant_pods_by_prefixes(
                relevant_pods,
                pod_name_prefixes=pod_name_prefixes,
            )

            if not relevant_pods:
                if pod_name_prefixes:
                    joined_prefixes = ", ".join(pod_name_prefixes)
                    last_issue = (
                        f"no relevant pods found in namespace '{namespace}' "
                        f"for prefixes: {joined_prefixes}"
                    )
                else:
    ...
    def verify_dataspace_ready_for_level4(self):
        ...
        if not self.wait_for_namespace_stability(
            ...
            duration=12,
            poll_interval=3,
            pod_name_prefixes=dataspace_prefixes,
        ):
    ...
    def deploy_infrastructure(self):
        ...
        # Fresh installs can take several minutes because keycloak config hooks
        # and stateful pods need extra startup time.
        initial_common_timeout = max(int(getattr(self.config, "TIMEOUT_POD_WAIT", 120)), 180)
        common_services_deployed = self.deploy_helm_release(
            ...
            cwd=common_dir,
            wait=False,
            timeout_seconds=None if common_release_exists else initial_common_timeout,
        )
