...
minikube_profile_exists() {
  ...
  minikube profile list --output json 2>/dev/null | grep -Fq "\"Name\":\"$profile\""
}
...
prune_component_images_before_build() {
  ...
  if [[ "$SKIP_MINIKUBE_LOAD" -eq 0 ]] && minikube_profile_exists "$MINIKUBE_PROFILE"; then
    local mini_refs
...
is_full_initial_manifest_target() {
  [[ "$MODE" == "initial" ]] || return 1
  [[ "${#TARGET_COMPONENTS[@]}" -eq "${#COMPONENT_KEYS[@]}" ]] || return 1

  local component
  local selected
  for component in "${COMPONENT_KEYS[@]}"; do
    for selected in "${TARGET_COMPONENTS[@]}"; do
...
resolve_manifest_file() {
  local canonical_manifest="$MANIFESTS_DIR/images-fast-step1.tsv"
  local partial_manifest="$MANIFESTS_DIR/images-fast-step1-partial-$(date -u +%Y-%m-%dT%H-%M-%SZ).tsv"

  if [[ -z "$MANIFEST_FILE" ]]; then
    if is_full_initial_manifest_target; then
      MANIFEST_FILE="$canonical_manifest"
    else
      MANIFEST_FILE="$partial_manifest"
    fi
    return
  fi

  if [[ "$MANIFEST_FILE" == "$canonical_manifest" ]] && ! is_full_initial_manifest_target; then
    echo "Canonical manifest path is reserved for full initial builds. Writing partial build manifest to $partial_manifest"
    MANIFEST_FILE="$partial_manifest"
  fi
}

if [[ "$MODE" == "initial" ]]; then
  if [[ "$SKIP_MINIKUBE_LOAD" -eq 1 ]]; then
    echo "Initial mode: full clean of INESData images in local Docker (minikube cleanup skipped)"
  else
