...
is_image_already_deployed() {
  local full_image="$1"
  local deployed_images=""

  deployed_images="$(kubectl get deploy -n "$K8S_NAMESPACE" -o jsonpath='{range .items[*]}{range .spec.template.spec.initContainers[*]}{.image}{"\n"}{end}{range .spec.template.spec.containers[*]}{.image}{"\n"}{end}{end}' 2>/dev/null || true)"

  if [[ -z "$deployed_images" ]]; then
    return 1
  fi

  echo "$deployed_images" | grep -Fxq "$full_image"
}
...
load_image_into_minikube() {
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    echo "[DRY-RUN] minikube -p \"$MINIKUBE_PROFILE\" image rm \"$full_image\""
    echo "[DRY-RUN] minikube -p \"$MINIKUBE_PROFILE\" ssh \"docker rmi -f $full_image >/dev/null 2>&1 || true\""
    echo "[DRY-RUN] minikube -p \"$MINIKUBE_PROFILE\" image load \"$full_image\""
    return 0
  fi

  if is_image_already_deployed "$full_image"; then
    echo "Image already deployed in namespace $K8S_NAMESPACE; forcing minikube reload for $full_image"
  fi

  echo "Refreshing minikube image cache for $full_image"
  minikube -p "$MINIKUBE_PROFILE" image rm "$full_image" >/dev/null 2>&1 || true
  minikube -p "$MINIKUBE_PROFILE" ssh "docker rmi -f $full_image >/dev/null 2>&1 || true" >/dev/null 2>&1 || true

  set +e
  load_output="$(minikube -p "$MINIKUBE_PROFILE" image load "$full_image" 2>&1)"
  load_rc=$?
  set -e
  ...
  if [[ "$load_rc" -eq 137 || "$load_output" == *"Killed"* ]]; then
    echo "minikube image load was killed while loading $full_image. Retrying with streamed docker load fallback..." >&2
  else
    echo "minikube image load failed for $full_image. Retrying with streamed docker load fallback..." >&2
  fi
  [[ -n "$load_output" ]] && echo "$load_output" >&2

  if ! docker image inspect "$full_image" >/dev/null 2>&1; then
    echo "Local Docker image not found for fallback: $full_image" >&2
    return "$load_rc"
  fi
  ...
  set +e
  fallback_output="$(docker save "$full_image" | minikube -p "$MINIKUBE_PROFILE" ssh "docker load" 2>&1)"
  fallback_rc=$?
  set -e
  ...
  echo "Fallback docker load also failed for $full_image" >&2
  [[ -n "$fallback_output" ]] && echo "$fallback_output" >&2
  return "$fallback_rc"
}
...
for key in "${load_keys[@]}"; do
  ...
  echo "$key -> $full_image"
  load_image_into_minikube "$full_image"
done
