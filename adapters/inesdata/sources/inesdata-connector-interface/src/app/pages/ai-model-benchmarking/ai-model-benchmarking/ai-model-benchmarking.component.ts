...
type UseCaseFamily = 'flares' | 'mobility';
...
export class AiModelBenchmarkingComponent implements OnInit {
  ...
  private readonly useCaseBenchmarkBatchSize = 64;
  private readonly benchmarkRequestTimeoutMs = 10000;
  private readonly useCaseBenchmarkRequestTimeoutMs = 45000;
  private validationDatasetSource: 'manual' | 'preset' | null = null;
  private readonly csvStringColumns = new Set([
    'trip_id',
    'journey_id',
    'from_stop_id',
    'to_stop_id',
  ...
  toggleAssetSelection(asset: BenchmarkAsset): void {
    ...
    if (!this.isAssetSelected(asset) && !this.isCompatibleWithCurrentSelection(asset)) {
      ...
      this.notificationService.showWarning(`This model is not comparable with ${anchorName}. Select models with the same input schema or from the same FLARES/Mobility use-case family.`);
      return;
    }
  ...
  private inferModelTask(model: BenchmarkAsset): ModelTask {
    const family = this.getUseCaseFamily(model);
    if (family === 'flares') return 'nlp';
    if (family === 'mobility') return 'regression';
  ...
  private getAvailableMetricsForSelection(models: BenchmarkAsset[]): string[] {
    const family = this.getSelectionUseCaseFamily(models);
    if (family === 'flares') {
    ...
    return this.metricsConfig[task] || this.metricsConfig.other;
  }
  ...
  getModelPoolSubtitle(): string {
    const anchorModel = this.getSelectionAnchorModel();
    if (!anchorModel) {
      return 'Select 2+ HTTP models with compatible inputs or the same FLARES/Mobility family';
    }

    const family = this.getUseCaseFamily(anchorModel);
    if (family === 'flares') {
      return `FLARES models comparable with ${anchorModel.name}`;
    }
    if (family === 'mobility') {
      return `Mobility models comparable with ${anchorModel.name}`;
    }
  ...
  getValidationDatasetSubtitle(): string {
    ...
    const family = this.getUseCaseFamily(anchorModel);
    if (family === 'flares') {
      return 'Suggested FLARES validation CSV with span and reliability targets, with manual upload still available.';
    }
    if (family === 'mobility') {
      return 'Suggested Mobility validation CSV with actual travel time, delay and previous delay targets, with manual upload still available.';
    }
  ...
  private isSchemaCompatible(referenceModel: BenchmarkAsset, candidateModel: BenchmarkAsset): boolean {
    ...
    const referenceFamily = this.getUseCaseFamily(referenceModel);
    const candidateFamily = this.getUseCaseFamily(candidateModel);
    if (referenceFamily && referenceFamily === candidateFamily) {
      return true;
    }
  ...
  private buildSuggestedDatasetRows(model: BenchmarkAsset): any[] {
    const family = this.getUseCaseFamily(model);
    if (family === 'flares') {
      return this.buildFlaresSuggestedRows();
    }
    if (family === 'mobility') {
      return this.buildMobilitySuggestedRows();
    }
  ...
  private buildFlaresSuggestedRows(): any[] {
    const examples = [
      {
        Id: 840,
        Text: 'El comite de medicamentos humanos espera concluir el analisis en marzo.',
        Tag_Start: 0,
        Tag_End: 33,
        '5W1H_Label': 'WHO',
      ...
      {
        Id: 841,
        Text: 'La reunion del consejo comenzara el lunes en Valencia.',
        Tag_Start: 33,
        Tag_End: 41,
        '5W1H_Label': 'WHEN',
        Tag_Text: 'el lunes',
      ...
      {
        Id: 842,
        Text: 'El sensor detecto una averia por humedad en la estacion norte.',
        Tag_Start: 18,
        Tag_End: 28,
        '5W1H_Label': 'WHAT',
        Tag_Text: 'una averia',
      ...
      {
        Id: 843,
        Text: 'Los tecnicos revisaron la red en el tunel central.',
        Tag_Start: 29,
        Tag_End: 48,
        '5W1H_Label': 'WHERE',
        Tag_Text: 'en el tunel central',
        Reliability_Label: 'confiable'
      },
      {
        Id: 844,
        Text: 'La alerta se activo por una subida repentina de temperatura.',
        Tag_Start: 20,
        Tag_End: 59,
        '5W1H_Label': 'WHY',
        Tag_Text: 'por una subida repentina de temperatura',
        Reliability_Label: 'no confiable'
      },
      {
        Id: 845,
        Text: 'El operador reinicio el servicio con un protocolo manual.',
        Tag_Start: 33,
        Tag_End: 56,
        '5W1H_Label': 'HOW',
        Tag_Text: 'con un protocolo manual',
        Reliability_Label: 'semiconfiable'
      },
      {
        Id: 846,
        Text: 'La empresa notifico a los usuarios afectados esta manana.',
        Tag_Start: 0,
        Tag_End: 10,
        '5W1H_Label': 'WHO',
        Tag_Text: 'La empresa',
      ...
      {
        Id: 847,
        Text: 'El sistema predijo retrasos de veinte minutos.',
        Tag_Start: 19,
        Tag_End: 45,
        '5W1H_Label': 'WHAT',
        Tag_Text: 'retrasos de veinte minutos',
        Reliability_Label: 'semiconfiable'
      }
    ];
  ...
  private buildMobilitySuggestedRows(): any[] {
    return Array.from({ length: 120 }, (_, index) => {
      const hour = (5 + (index % 18)) % 24;
      const weekday = index % 7;
      const routeNumber = 10 + (index % 8);
      const scheduledTravelTime = 90 + ((index * 17) % 240);
      const shapeDistance = this.roundNumber(420 + ((index * 53) % 1600) + ((index % 5) * 12.5), 6);
      const isPeak = [7, 8, 9, 17, 18, 19].includes(hour) ? 1 : 0;
      const hourAngle = (2 * Math.PI * hour) / 24;
      const weekdayAngle = (2 * Math.PI * weekday) / 7;
      const baseDelay = ((index % 9) - 3) * 4;
      const peakDelay = isPeak ? 16 + (index % 5) * 2 : 0;
      const distanceDelay = Math.round(shapeDistance / 400) - 2;
      const delay = this.roundNumber(baseDelay + peakDelay + distanceDelay, 3);
      const previousDelay = this.roundNumber(delay - 3 + ((index % 6) * 1.5), 3);
      const previousDelayRatio = this.roundNumber(previousDelay / Math.max(scheduledTravelTime, 1), 9);
      const previousDelayDelta = this.roundNumber(previousDelay - (((index + 2) % 7) * 2), 3);
      const actualTravelTime = this.roundNumber(Math.max(15, scheduledTravelTime + delay), 3);

      return {
        trip_id: `L${routeNumber}_${Math.floor(index / 8) + 1}_${String(hour).padStart(2, '0')}:${String((index * 7) % 60).padStart(2, '0')}_PIONERA`,
        from_stop_id: String(7000 + ((index * 13) % 900)),
        to_stop_id: String(19000 + ((index * 19) % 1100)),
        route_id: String(routeNumber),
        scheduled_travel_time: scheduledTravelTime,
        shape_distance: shapeDistance,
        is_peak: isPeak,
        hour_sin: this.roundNumber(Math.sin(hourAngle), 12),
        hour_cos: this.roundNumber(Math.cos(hourAngle), 12),
        weekday_sin: this.roundNumber(Math.sin(weekdayAngle), 12),
        weekday_cos: this.roundNumber(Math.cos(weekdayAngle), 12),
        previous_delay_ratio: previousDelayRatio,
        previous_delay_delta: previousDelayDelta,
        actual_travel_time: actualTravelTime,
        delay,
        previous_delay: previousDelay
      };
    });
  }
  ...
  private getPreferredRegressionKeys(model: BenchmarkAsset): string[] {
    ...
    if (this.isMobilityPreviousDelayModel(model)) return ['previous_delay'];
    if (this.isMobilityActualTravelTimeModel(model)) return ['actual_travel_time'];
    if (this.isMobilityDelayModel(model)) return ['delay'];
  ...
  private getPreferredClassificationKeys(model: BenchmarkAsset): string[] {
    ...
    if (this.isFlaresReliabilityModel(model)) return ['Reliability_Label', 'reliability_label'];
    if (this.isFlares5w1hModel(model)) return ['5W1H_Label', 'Label_5W1H', 'label'];
  ...
  private getUseCaseFamily(model: BenchmarkAsset): UseCaseFamily | null {
    const identity = this.getModelIdentity(model);
    if (identity.includes('flares') || identity.includes('5w1h')) {
      return 'flares';
    }
    if (
      identity.includes('mobility')
      || identity.includes('actual-travel-time')
      || identity.includes('actual_travel_time')
      || identity.includes('actual travel time')
      || identity.includes('previous-delay')
      || identity.includes('previous_delay')
      || identity.includes('previous delay')
    ) {
      return 'mobility';
    }
  ...
  private getSelectionUseCaseFamily(models: BenchmarkAsset[]): UseCaseFamily | null {
    if (models.length === 0) {
      return null;
    }

    const families = models.map(model => this.getUseCaseFamily(model));
    const firstFamily = families[0];
    return firstFamily && families.every(family => family === firstFamily) ? firstFamily : null;
  }

  private isFlares5w1hModel(model: BenchmarkAsset): boolean {
    const identity = this.getModelIdentity(model);
    return this.getUseCaseFamily(model) === 'flares' && identity.includes('5w1h');
  }

  private isFlaresReliabilityModel(model: BenchmarkAsset): boolean {
    const identity = this.getModelIdentity(model);
    return this.getUseCaseFamily(model) === 'flares' && identity.includes('reliability');
  }

  private isMobilityPreviousDelayModel(model: BenchmarkAsset): boolean {
    const identity = this.getModelIdentity(model);
    return this.getUseCaseFamily(model) === 'mobility'
      && (identity.includes('previous-delay') || identity.includes('previous_delay') || identity.includes('previous delay'));
  }

  private isMobilityActualTravelTimeModel(model: BenchmarkAsset): boolean {
    const identity = this.getModelIdentity(model);
    return this.getUseCaseFamily(model) === 'mobility'
      && (identity.includes('actual-travel-time') || identity.includes('actual_travel_time') || identity.includes('actual travel time'));
  }

  private isMobilityDelayModel(model: BenchmarkAsset): boolean {
    const identity = this.getModelIdentity(model);
    return this.getUseCaseFamily(model) === 'mobility'
      && identity.includes('delay')
      && !this.isMobilityPreviousDelayModel(model);
  }
    ...
    try {
      for (const model of selectedModels) {
        ...
        if (this.inputMode === 'single') {
          try {
            const response = await this.executeModel(model.id, this.normalizeExecutionPayload(model, normalizedInput));
          ...
          for (const row of rows) {
            try {
              const response = await this.executeModel(model.id, this.normalizeExecutionPayload(model, row));
  ...
  private async executeBenchmarkRowsForModel(
      ...
      const settledBatch = useBatchExecution
        ...
        : await Promise.all(
          batchRows.map(async (row, rowOffset) => {
            ...
            try {
              const result = await this.executeModel(model.id, this.normalizeExecutionPayload(model, row), this.benchmarkRequestTimeoutMs, benchmarkRunId, correlationId, model.name);
    ...
    const comparablePairCount = this.isFlares5w1hModel(model)
      ? this.buildFlaresSpanPairs(datasetRows, predictions).length
      : this.detectedTask === 'regression'
  ...
  private async executeBenchmarkBatchForModel(
    ...
    try {
      const result = await this.executeModel(
        model.id,
        this.normalizeExecutionPayload(model, batchRows),
  ...
  private normalizeExecutionPayload(model: BenchmarkAsset, input: any): any {
    ...
    const family = this.getUseCaseFamily(model);
    if ((family === 'flares' || family === 'mobility') && !Array.isArray(executionInput)) {
  ...
  private buildExecutionRecordForModel(model: BenchmarkAsset, row: any): any {
    ...
    const fields = this.getExecutionInputFieldsForModel(model);
    if (fields.length === 0) {
      return row;
    }
  ...
  private getExecutionInputFieldsForModel(model: BenchmarkAsset): SchemaField[] {
    if (this.getUseCaseFamily(model) === 'mobility') {
      return this.getMobilityExecutionInputFields(model);
    }

    return this.getSchemaFieldsFromModel(model);
  }

  private getMobilityExecutionInputFields(model: BenchmarkAsset): SchemaField[] {
    const commonFields: SchemaField[] = [
      { name: 'trip_id', type: 'string', required: true },
      { name: 'from_stop_id', type: 'string', required: true },
      { name: 'to_stop_id', type: 'string', required: true },
      { name: 'route_id', type: 'string', required: true },
      { name: 'scheduled_travel_time', type: 'number', required: true },
      { name: 'shape_distance', type: 'number', required: true },
      { name: 'is_peak', type: 'integer', required: true, min: 0, max: 1 },
      { name: 'hour_sin', type: 'number', required: true },
      { name: 'hour_cos', type: 'number', required: true },
      { name: 'weekday_sin', type: 'number', required: true },
      { name: 'weekday_cos', type: 'number', required: true }
    ];

    if (this.isMobilityPreviousDelayModel(model)) {
      return commonFields;
    }

    return [
      ...commonFields,
      { name: 'previous_delay_ratio', type: 'number', required: true },
      { name: 'previous_delay_delta', type: 'number', required: true }
    ];
  }
  ...
  private calculateMetrics(model: BenchmarkAsset, datasetRows: any[], predictions: any[], taskType: ModelTask): Record<string, number> {
    ...
    const flaresSpanMetrics = this.isFlares5w1hModel(model)
      ? this.computeFlaresSpanMetrics(datasetRows, predictions)
      : null;

    const accuracy = this.computeClassificationAccuracy(classificationPairs);
    const precision = flaresSpanMetrics?.precision ?? this.computeMacroPrecision(classificationPairs);
    const recall = flaresSpanMetrics?.recall ?? this.computeMacroRecall(classificationPairs);
    const f1Score = flaresSpanMetrics?.f1 ?? this.computeMacroF1(classificationPairs);
  ...
  private buildFlaresSpanPairs(
    datasetRows: any[],
    predictions: any[]
  ): Array<{ actual: FlaresSpan; predicted: FlaresSpan | null; score: number }> {
    const pairs: Array<{ actual: FlaresSpan; predicted: FlaresSpan | null; score: number }> = [];

    datasetRows.forEach((row, index) => {
      const actual = this.extractExpectedFlaresSpan(row);
      if (!actual) {
        return;
      }

      const predictedSpans = this.extractPredictedFlaresSpans(predictions[index]);
      let bestPredicted: FlaresSpan | null = null;
      let bestScore = 0;
      predictedSpans.forEach(predicted => {
        const score = this.scoreFlaresSpanMatch(actual, predicted);
        if (score > bestScore) {
          bestScore = score;
      ...
      pairs.push({ actual, predicted: bestPredicted, score: bestScore });
    });

    return pairs;
  }

  private computeFlaresSpanMetrics(datasetRows: any[], predictions: any[]): { precision: number; recall: number; f1: number } {
    let expectedCount = 0;
    let predictedCount = 0;
    let matchCredit = 0;

    datasetRows.forEach((row, index) => {
      const expectedSpan = this.extractExpectedFlaresSpan(row);
      const predictedSpans = this.extractPredictedFlaresSpans(predictions[index]);
      predictedCount += predictedSpans.length;
      ...
      expectedCount += 1;
      const bestScore = predictedSpans.reduce((best, predicted) => Math.max(best, this.scoreFlaresSpanMatch(expectedSpan, predicted)), 0);
      matchCredit += bestScore;
    });

    const precision = matchCredit / Math.max(predictedCount, 1);
    const recall = matchCredit / Math.max(expectedCount, 1);
    const f1 = (precision + recall) === 0 ? 0 : (2 * precision * recall) / (precision + recall);
    return { precision, recall, f1 };
  }

  private extractExpectedFlaresSpan(row: any): FlaresSpan | null {
    if (!row || typeof row !== 'object') {
      return null;
    }

    const start = this.readNumericValue(row.Tag_Start ?? row.start);
    const end = this.readNumericValue(row.Tag_End ?? row.end);
    const label = this.normalizeFlaresSpanLabel(row['5W1H_Label'] ?? row.Label_5W1H ?? row.label);
    ...
    return {
      ...
      label,
      text: row.Tag_Text ? String(row.Tag_Text) : undefined
    };
  }

  private extractPredictedFlaresSpans(output: any): FlaresSpan[] {
    if (output === undefined || output === null) {
      return [];
    }

    if (Array.isArray(output)) {
      const spans: FlaresSpan[] = [];
      output.forEach(item => {
        spans.push(...this.extractPredictedFlaresSpans(item));
      });
      return spans;
    }
    ...
    const start = this.readNumericValue(output.Tag_Start ?? output.start);
    const end = this.readNumericValue(output.Tag_End ?? output.end);
    const label = this.normalizeFlaresSpanLabel(output['5W1H_Label'] ?? output.Label_5W1H ?? output.label);
    if (!Number.isFinite(start) || !Number.isFinite(end) || !label) {
      return [];
    }
    ...
    return [{
      ...
      label,
      text: output.Tag_Text ? String(output.Tag_Text) : undefined
    }];
  }

  private scoreFlaresSpanMatch(expected: FlaresSpan, predicted: FlaresSpan): number {
    if (expected.label !== predicted.label) {
      return 0;
    }

    if (expected.start === predicted.start && expected.end === predicted.end) {
      return 1;
    }

    const overlaps = predicted.end > expected.start && predicted.start < expected.end;
    return overlaps ? 0.5 : 0;
  }

  private normalizeFlaresSpanLabel(value: any): string {
    if (value === undefined || value === null || value === '') {
      return '';
    }

    return String(value).replace(/^B-/, '').replace(/^I-/, '').trim().toUpperCase();
  }
  ...
  private extractPredictionScore(output: any, predictedValue: string): number {
    if (Array.isArray(output)) {
      for (const item of output) {
        const score = this.extractPredictionScore(item, predictedValue);
        if (score > 0) {
          return score;
        }
      }
  ...
  private initializeStandardizedSingleInput(models: BenchmarkAsset[]): void {
    ...
    const selectedFamily = models.length > 1 ? this.getSelectionUseCaseFamily(models) : null;
    if (selectedFamily) {
      this.standardizedInputFields = this.getUnifiedUseCaseInputFields(selectedFamily);
      const nextValues: Record<string, any> = {};
      const exampleValues = this.getUnifiedUseCaseInputExample(selectedFamily);
      this.standardizedInputFields.forEach(field => {
  ...
  private getUnifiedUseCaseInputFields(family: UseCaseFamily): SchemaField[] {
    if (family === 'flares') {
      return [
        { name: 'Id', type: 'integer', required: true, description: 'Input text identifier' },
        { name: 'Text', type: 'string', required: true, description: 'Spanish text to analyze' },
        { name: 'Tag_Start', type: 'integer', required: true, description: 'Span start character offset' },
        { name: 'Tag_End', type: 'integer', required: true, description: 'Span end character offset' },
        { name: '5W1H_Label', type: 'string', required: true, description: 'Expected 5W1H span label' },
        { name: 'Tag_Text', type: 'string', required: true, description: 'Extracted span text' }
      ];
    }

    return [
      { name: 'trip_id', type: 'string', required: true, description: 'GTFS trip identifier' },
      { name: 'from_stop_id', type: 'string', required: true, description: 'Origin stop identifier' },
      { name: 'to_stop_id', type: 'string', required: true, description: 'Destination stop identifier' },
      { name: 'route_id', type: 'string', required: true, description: 'GTFS route identifier' },
      { name: 'scheduled_travel_time', type: 'number', required: true, description: 'Scheduled segment travel time in seconds' },
      { name: 'shape_distance', type: 'number', required: true, description: 'Segment distance in meters' },
      { name: 'is_peak', type: 'integer', required: true, min: 0, max: 1, description: 'Peak-hour indicator' },
      { name: 'hour_sin', type: 'number', required: true, description: 'Cyclic hour sine encoding' },
      { name: 'hour_cos', type: 'number', required: true, description: 'Cyclic hour cosine encoding' },
      { name: 'weekday_sin', type: 'number', required: true, description: 'Cyclic weekday sine encoding' },
      { name: 'weekday_cos', type: 'number', required: true, description: 'Cyclic weekday cosine encoding' },
      { name: 'previous_delay_ratio', type: 'number', required: true, description: 'Previous delay divided by scheduled travel time' },
      { name: 'previous_delay_delta', type: 'number', required: true, description: 'Previous delay delta in seconds' }
    ];
  }

  private getUnifiedUseCaseInputExample(family: UseCaseFamily): Record<string, any> {
    if (family === 'flares') {
      return {
        Id: 840,
        Text: 'El comite de medicamentos humanos espera concluir el analisis en marzo.',
    ...
    return {
      ...
      shape_distance: 681.19568488104,
      is_peak: 0,
      hour_sin: 0.707106781187,
      hour_cos: 0.707106781187,
      weekday_sin: 0.974927912182,
      weekday_cos: -0.222520933956,
      previous_delay_ratio: 0.249999997917,
      previous_delay_delta: 0
    };
  }
  ...
  private shouldPreserveCsvValueAsString(header: string): boolean {
    ...
    return this.csvStringColumns.has(normalized) || normalized.endsWith('_id');
  }
