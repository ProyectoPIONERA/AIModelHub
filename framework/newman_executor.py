...
class NewmanExecutor:
    ...
    def _npm_env_candidates(self):
        base_env = os.environ.copy()
        yield base_env

        debian_node_path = "/usr/share/nodejs"
        if os.path.isdir(debian_node_path):
            env = base_env.copy()
            current_node_path = env.get("NODE_PATH", "")
            paths = [item for item in current_node_path.split(os.pathsep) if item]
            if debian_node_path not in paths:
                paths.insert(0, debian_node_path)
            env["NODE_PATH"] = os.pathsep.join(paths)
            yield env

    def _resolve_working_npm(self):
        npm_cmd = shutil.which("npm")
        if not npm_cmd:
            return None, None, "npm executable was not found in PATH"

        last_error = ""
        for env in self._npm_env_candidates():
        ...
        return npm_cmd, None, last_error or "npm --version failed"
    ...
    def ensure_available(self):
        ...
        if os.path.exists(package_json):
            npm_cmd, npm_env, npm_error = self._resolve_working_npm()
            if npm_cmd is None or npm_env is None:
                raise RuntimeError(
                    "Newman is not available and npm could not be used to install it. "
                    f"npm diagnostic: {npm_error}"
                )

            install_command = [npm_cmd, "ci", "--no-audit", "--no-fund"]
            if not os.path.exists("package-lock.json"):
                install_command = [npm_cmd, "install", "--no-audit", "--no-fund"]
            ...
            if result.returncode != 0:
                ...
                raise RuntimeError(
                    "Failed to install local Newman dependency with npm. "
        ...
        raise RuntimeError(
            "Newman is not installed or not available locally. "
            "Run npm ci in the project root or install Newman globally."
        )

    def resolve_newman_command(self):
        local_newman = os.path.join("node_modules", ".bin", "newman")
        if os.path.exists(local_newman):
            return self._local_newman_command(local_newman)
    ...
    def _local_newman_command(self, local_newman):
        if os.access(local_newman, os.X_OK):
            return [local_newman]

        if self._make_executable(local_newman):
            print(f"[INFO] Fixed execute permission for local Newman: {local_newman}")
            return [local_newman]

        node_cmd = shutil.which("node")
        if node_cmd:
            print(
                "[INFO] Local Newman is not executable; "
                f"running it through Node.js: {local_newman}"
            )
            return [node_cmd, local_newman]
    ...
    def _make_executable(self, path):
        try:
            ...
            if executable_mode != current_mode:
                os.chmod(target, executable_mode)
            return os.access(path, os.X_OK)
        except OSError as exc:
            print(f"[WARNING] Could not chmod local Newman executable '{path}': {exc}")
            return False

    def _permission_fallback_command(self, cmd):
        if not cmd:
            return None
        command_path = cmd[0]
        local_newman = os.path.join("node_modules", ".bin", "newman")
        if os.path.normpath(command_path) != os.path.normpath(local_newman):
            return None
        node_cmd = shutil.which("node")
        if not node_cmd:
            return None
        return [node_cmd, local_newman] + cmd[1:]
    ...
    def _write_environment_file(self, env_vars, environment_path):
        payload = {
            "id": "aimodelhub-pionera",
            "name": "AIModelHub Pionera Environment",
            "values": [
                {
                    "key": key,
                    "value": value,
    ...
    def run_newman(self, collection_path, env_vars, report_path=None, environment_path=None):
        ...
        try:
            ...
            raise RuntimeError(
                "Newman command disappeared before execution. "
                "Run npm ci in the project root and retry Step 6."
            )
        except PermissionError:
            fallback_cmd = self._permission_fallback_command(cmd)
            if not fallback_cmd:
                raise

            print("[WARNING] Newman local binary was not executable; retrying through Node.js")
            result = subprocess.run(
                fallback_cmd,
                check=False,
                capture_output=False,
