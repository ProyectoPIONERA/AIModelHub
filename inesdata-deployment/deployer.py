...
def create_database(pg_user, pg_password, pg_host, database, username, password):
    ...
    if cur.fetchone():
        cur.execute(
            sql.SQL("ALTER USER {} WITH ENCRYPTED PASSWORD %s").format(sql.Identifier(username)),
            (password,),
        )
    else:
        cur.execute(
...
def normalize_keycloak_audience_base(keycloak_url):
    """Normalize audience base URL used in Keycloak mapper claims.

    For local deployments, tokens are often requested from :8080 while connectors
    validate against the ingress hostname without that port. Keep scheme and host,
    but drop :8080 so mapper audience remains compatible with connector config.
    """
    value = (keycloak_url or '').strip().rstrip('/')
    if not value:
        return value
...
def create_realm(username, password, server_url, realm_name, dataspace_name, keycloak_url, environment):
    ...
    expected_audience = f"{normalize_keycloak_audience_base(keycloak_url)}/realms/{realm_name}"

    # Ensure the audience client scope exists and its mapper points to the
    # audience expected by connector oauth validation.
    click.echo(f'  + Creating scope "dataspaceunit-dataspace-audience"' )
    client_scopes = keycloak_admin.get_client_scopes()
    dataspace_scope = next((scope for scope in client_scopes if scope['name'] == 'dataspaceunit-dataspace-audience'), None)

    if not dataspace_scope:
        ...
        dataspace_scope = keycloak_admin.get_client_scope_by_name('dataspaceunit-dataspace-audience')
    ...
    mappers = keycloak_admin.get_mappers_from_client_scope(dataspace_scope['id'])
    audience_mapper = next((mapper for mapper in mappers if mapper.get('name') == 'add-namespace-audience'), None)
    if not audience_mapper:
        keycloak_admin.add_mapper_to_client_scope(dataspace_scope['id'], mapper_payload)
    else:
        current_audience = (audience_mapper.get('config') or {}).get('included.custom.audience')
        if current_audience != expected_audience:
            click.echo(f'    + Updating audience mapper to {expected_audience}')
            audience_mapper['config'] = audience_mapper.get('config') or {}
            audience_mapper['config']['included.custom.audience'] = expected_audience
